...
{                                                                               
   ...
   printf("\r\nKernel name: %s\r\nKernel version: %s\r\nKernel build date: %s %s\r\nUptime: %u hours %u minutes %u seconds\r\nCPU model number: %u\r\n", KERNEL_NAME, KERNEL_VERS, KENREL_DATE, KERNEL_TIME, hr, min, sec, get_model());
   bg_color = RED;
   printf(" ");
   bg_color = GREEN;
   printf(" ");
   ...
   while (true)
   {
      ...
      else if (check_cmd("mandelbrot"))
      {
         if (argc == 5)
         {
            ...
            // else
            // {
            mandelbrot(atof(argv[1]), atof(argv[2]), atof(argv[3]), atof(argv[4]), 0x000000);
            //    }
         }
      ...
      {
         printf("Warning this function will BSOD (Blue Screen of Death) your system. This is usually bad.\r\nContinue y/[N]: ");
         char *confirm = gets();
